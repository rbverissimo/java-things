...
public class FarmaciasApiController extends ControllerCommons {
    ...
    @PostMapping("/share")
    public ResponseEntity<?> compartilharFarmacia(@Valid ShareFarmacia shareFarmacia){
       ...
       if(!authService.usuarioByEmail(shareFarmacia.getEmailUsuario()).isPresent()) return ResponseEntity.badRequest()
